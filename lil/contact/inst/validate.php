...
if ($_POST) {
  ...
  /*if($birth_y == null || $birth_m == null || $birth_d == null){
    $errFlag = true; $errMsg['birth']  ="<label class='validError'>生年月日を入力してください。</label>";
  }elseif (!chkNum($birth_y) || !checkdate($birth_m, $birth_d, $birth_y)) {
    $errFlag = true; $errMsg['birth']  ="<label class='validError'>正しい生年月日を入力してください。</label>";
  }*/
  // 電話番号
  if($tel1 == null || $tel2 == null || $tel3 == null) {
    $errFlag = true; $errMsg['tel'] = "<label class='validError'>電話番号を入力してください。</label>";
  }elseif(!chkTel($tel)){
